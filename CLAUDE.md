...
- **Config overrides** are how the global admin tunes what boards inherit, at two levels that stack: `data/boardDefaults.log` (every board) and `boards/<uri>/data/boardConfig.log` (that board only), both one `key<>value` line per override and both read by [configOverrideRepository.php](code/TwintailUploader/Classes/configOverrideRepository.php). `applyLayers()` lays them over the global `$conf` in that order before `applyToConfig()` — `warota.php`, `expireFiles.php` and `recentFilesRepository` all go through it — and a key absent from both falls back to `config.php`. `KEYS` is the whitelist (limits, cooldown, listing, temporary hosting, unlisted, action log, default theme); values are coerced to the type `config.php` gives the key on save *and* on read, and a theme name is checked against the installed themes on save. The *Board defaults* mod page (`modPage=boardDefaults`) edits the first, the board-scoped *Board config* page (`boards/<uri>/index.php?request=admin&modPage=config`, refused to owners) edits the second; both render through `drawConfigOverrideEditor()`, which shows the layer below as the fallback. Adding a key = an entry in `KEYS`; it must already exist as a scalar in `config.php`.
...
The visitor's display preferences are the one thing that is **not** `<>`-delimited: [cookieSettingsManager.php](code/TwintailUploader/Classes/cookieSettingsManager.php) stores them as a JSON object keyed by setting name, scoped to the instance root (`instanceCookiePath()` in [Functions/request.php](code/TwintailUploader/Functions/request.php) climbs `rootScript`'s `../` hops) so the main uploader and every board share one cookie; a board-scoped cookie from before is expired on the next write. The set of settings is whatever `$conf['defaultCookieValues']` declares — add a key there and it is read, written and defaulted automatically. Values are normalised to `'checked'`/`''` on the way out, so a hand-edited cookie can't reach the markup. Legacy positional cookies are still decoded once and rewritten as JSON.
...
`allowUserBoards` turns the user-board feature on: the nav link, the listing and the creation page all go with it (existing board directories stay reachable). `allowBoardCreation` only stops new boards being made.
