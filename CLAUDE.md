...
- **Board defaults** are the admin's overrides for every board at once: `data/boardDefaults.log`, one `key<>value` line per override, owned by [boardDefaultsRepository.php](code/TwintailUploader/Classes/boardDefaultsRepository.php). It is a layer between `config.php` and `applyToConfig()` — `warota.php`, `expireFiles.php` and `recentFilesRepository` all call `apply()` on the global `$conf` before handing it to the board — and a key that is absent falls back to `config.php`. `KEYS` is the whitelist (limits, cooldown, listing, temporary hosting, unlisted, action log, default theme); values are coerced to the type `config.php` gives the key on save *and* on read, and a theme name is checked against the installed themes on save. Edited from the *Board defaults* mod page (`modPage=boardDefaults`). Adding a key = an entry in `KEYS`; it must already exist as a scalar in `config.php`.
