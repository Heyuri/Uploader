...
return $conf = [
    ...
    'chunkSize' => 20*1024*1024,           // chunk size in bytes for chunked uploads (must be under server upload limit, e.g. nginx client_max_body_size) / 分割アップロードのチャンクサイズ（バイト）。サーバーのアップロード制限（例: nginx client_max_body_size）以下にすること
