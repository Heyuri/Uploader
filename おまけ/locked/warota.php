...
// 基本設定-----------------------------------------------------------------
  $page_title   = 'Title';          // Page titile.
  $page_desc = 'Go team sea slug!'; //Page description.
  $iprec = 1; //Whether or not to record IP addresses.
  $logfile      = 'souko.log';          // Log file.
  $logmax       = 200;                  // Max amount of posts on the board.
  $limitk       = 1000*1024;               //Filesize limit in mb.
  $max_all_flag = 1;                    // Not implemented.
  $max_all_size = 200000;                   //Max size of all files on the board.
  $updir        = './src/';             // Directory where files are uploaded.
  ...
  // ▼Yakuba(設定追加)
  $b_changeext  = array('mht','cgi','php','html','sh'); //Filetypes to convert.
  $a_changeext  = 'txt';                // Filetype to convert above files to.
  $base_php     = 'warota.php';          // Name of this php file.
  $homepage_add = '../index.html';      // Location of homepage.
  // ▲Yakuba
