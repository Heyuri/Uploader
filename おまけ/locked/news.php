...
// 基本設定-----------------------------------------------------------------
  $page_title   = 'News';          // Page titile.
  $page_desc = '<a href="https://knn.jp.net/">KNN</a>'; //Page description.
  $iprec = 0; //Whether or not to record IP addresses.
  $logfile      = 'newssouko.log';          // Log file.
  $logmax       = 1000;                  // Max amount of posts on the board.
  $limitk       = 100*1024;               //Filesize limit in mb.
  $max_all_flag = 1;                    // Not implemented.
  $max_all_size = 10000;                   //Max size of all files on the board.
  $updir        = './newssrc/';             // Directory where files are uploaded.
  $prefix       = 'news';                   // File prefix（up001.txt,up002.jpgならup）.
  ...
  $last_file    = 'last.log';           // File for limiting continous posting.
  ...
  $arrowext     = array('bmp','cgi','gif','jpg','png','txt','mht','htm','html', 'rar', 'zip', '7z');    //Allow extensions (these must be in lowercase or else an error will be given)
    //Allowed filetypes.

  // ▼Yakuba(設定追加)
  $b_changeext  = array('mht','cgi','php','html','sh','htm','shtml'); //Filetypes to convert.
  $a_changeext  = 'txt';                // Filetype to convert above files to.
  $base_php     = 'news.php';          // Name of this php file.
  $homepage_add = '../..';      // Location of homepage.
  // ▲Yakuba
...
<style>
  ...
  tr:nth-child(odd) {background-color: #CFC;}
  tr:hover {background-color: transparent;}
  table {border-collapse: collapse;}
  -->
</style>
</head>
<body bgcolor="#ffffee" text="#000" link="#0000ee" alink="#5555ee" vlink="#0000ee" background="index.gif">
<table width="100%"><tr><td bgcolor="#CFC"><strong><font size="4">'.$page_title.'</font></strong></td></tr></table>
<tt>
...
if(file_exists($upfile) && $com && $upfile_size > 0){
  ...
  if(!in_array($ext, $arrowext))
    error("Error","This filetype is not allowed.");
