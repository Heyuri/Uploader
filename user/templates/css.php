...
// 基本設定-----------------------------------------------------------------
  include 'settingspass.php';
  $page_title   = 'CSS';          // タイトル名
  $logfile      = 'cssouko.log';          // ログファイル名(変更する事)
  $logmax       = 1;                  // 保持件数
  $limitk       = 1*1024;               // 最大投稿容量(KB)(＜upload_max_filesize ← 標準2M)
  $max_all_flag = 1;                    // 総容量規制を使用する=1(未実装)
  $max_all_size = 1;                   // 総制限容量(MB)
  $updir        = './csrc/';             // ファイル格納ディレクトリ
  $prefix       = '';                   // 接頭語（up001.txt,up002.jpgならup）
  $commax       = 250;                  // コメント投稿量制限（バイト。全角はこの半分）
  $page_def     = 1;                   // 一ページの表示行数
  $admin        = $password;               // 削除管理パス
  $auto_link    = 0;                    // コメントの自動リンク（Yes=1;No=0);
  $last_time    = 0;                    // 同一IPからの連続投稿許可する間隔(分)(0で無制限)
  $last_file    = 'clast.log';           // 連続投稿制限用ファイル(空ファイルで666)
  $count_look   = 0;                    // カウンタ表示(Yes=1,No=0)
  $count_file   = 'ccount.log';          // カウンタファイル(空ファイルで666)
  ...
  $arrowext     = array('css');    //Allow extensions (these must be in lowercase or else an error will be given)
    //許可拡張子 小文字（それ以外はエラー

  // ▼Yakuba(設定追加)
  $b_changeext  = array('htm','mht','cgi','php','html','sh');
  $a_changeext  = 'txt';                // 強制変換後の拡張子
  $base_php     = 'css.php';          // このファイル名
  $homepage_add = 'index.php';      // [Home]のリンク先(相対、絶対両方可能)
  // ▲Yakuba
...
<tt>
<br><br>【Upload a custom CSS.】<br><br><br>
</tt>
';
...
if(file_exists($upfile) && $com && $upfile_size > 0){
  ...
  if(!in_array($ext, $arrowext))
    error("Error","Disallowed file extension.");
  ...
  */
  ...
  /* 自鯖転送 */
  if($pass2 !== $postpass) error('Incorrect password.');
...
if($size_all_hikaku >= $max_all_size){
  ...
  DELKey: <INPUT TYPE=password SIZE="10" NAME="pass" maxlength="10"> Pass: <INPUT TYPE=password SIZE="10" NAME="pass2" maxlength="25"><br>
